...
public class MainGUI extends JFrame {
	...
	protected void make(GameData gameData, JLabel timerLabel, int teamIndex, List clock) {
		this.gameData = gameData;
	...
	private JPanel createJeopardyPanel() {
		...
		jeopardyPanel.setLayout(new FlowLayout());
		JLabel jeopardyLabel = new JLabel("Jeopardy");
		//timerLabel = new JLabel("");
		AppearanceSettings.setBackground(AppearanceConstants.lightBlue, jeopardyPanel, jeopardyLabel, timerLabel);
		jeopardyLabel.setFont(AppearanceConstants.fontLarge);
		timerLabel.setFont(AppearanceConstants.fontMedium);
		jeopardyPanel.add(jeopardyLabel);
		jeopardyPanel.add(timerLabel);
	...
	private JPanel createProgressPanel() {
		...
		JPanel updatesPanel = new JPanel(new FlowLayout());
		// other local variables
		JLabel updatesLabel = new JLabel("Game Progress");
		JLabel teamNameLabel = new JLabel(gameData.getTeamDataList().get(teamIndex).getMainTeamNameLabel().getText());
		...
		teamNameLabel.setFont(AppearanceConstants.fontMedium);
		pointsPanel.setBackground(Color.darkGray);
		updatesPanel.setBorder(BorderFactory.createLineBorder(Color.darkGray));
		updatesPanel.add(updatesLabel);
		updatesPanel.add(teamNameLabel);
		...
		southEastPanel.add(updatesPanel, BorderLayout.NORTH);
		...
		for (int i = 0; i < gameData.getNumberOfTeams(); i++) {
			...
			if (gameData.getCurrentTeam().getTeamName().equals(team.getTeamName())) {
				pointsPanel.add(clock.get(i).getLabel());
			}
			else {
				clock.get(i).setInvisible();
				pointsPanel.add(clock.get(i).getLabel());
			}
